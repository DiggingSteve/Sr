...
namespace Sr.Tests
{
    public abstract class SrTestBase : AbpIntegratedTestBase<SrTestModule>
    {
        ...
        protected void UsingDbContext(Action<SrDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SrDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SrDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SrDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SrDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SrDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SrDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SrDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SrDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SrDbContext, Task<T>> func)
        {
            T result;
